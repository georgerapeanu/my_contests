...
const int NMAX = 2e5;
const int targetN = 1e5;
const int LGMAX = 62;
const int LEN = 1e4;
const int Used_Nodes_st = 30;
const int Used_Nodes_dr = LGMAX - 1;
const int Random_Nodes = (NMAX >= 40000 ? NMAX / 1000 : 40);
const int Random_Querys = (NMAX >= 20000 ? NMAX / 2000 : 10);
...
void transform_set_base_on_Tree(Tree a,int how_many_bonus_nodes){
	...
	for(int i = 0;i < a.Used_Nodes && how_many_bonus_nodes;i++){
		for(long long mask = a.node_clique_mask[a.nodes[i]];mask && how_many_bonus_nodes;mask = (mask - 1) & a.node_clique_mask[a.nodes[i]]){
...
int main(){
	...
	for(int test = 21;test <= 25;test++){///departajare Nlog2N de Nlog3N
		vector< vector<long long> > BEST_INPUT;
		long long best_dif = -1;
		for(int epoch = 1;epoch <= 100;epoch++){
			generate_input(1e5);
			long long Nlog2N_start = clock();
			system("Nlog2N.exe");
			long long Nlog2N_end = clock();
			long long Nlog2N_duration = Nlog2N_end - Nlog2N_start;

			long long Nlog3N_start = clock();
			system("Nlog3N.exe");
			long long Nlog3N_end = clock();
			long long Nlog3N_duration = Nlog3N_end - Nlog3N_start;
			if(Nlog3N_duration - Nlog2N_duration > best_dif){
				best_dif = Nlog3N_duration - Nlog2N_duration;
				BEST_INPUT = INPUT;
			}
		}
